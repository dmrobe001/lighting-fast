...
async function loadTexture(device, url) {
    ...
    return new Promise((resolve) => {
        ...
        device.queue.copyExternalImageToTexture({ source: source, flipY: true }, { texture: texture }, { width: source.width, height: source.height });
        resolve(texture);
    });
}
//
...
async function initWebGPU() {
    ...
    const shaderModule = device.createShaderModule({ code: await fetch('path_tracing.wgsl').then((res) => res.text()), });
